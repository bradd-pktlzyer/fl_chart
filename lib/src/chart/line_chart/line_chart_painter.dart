  ...
  void drawBar(
    ...
    if (barData.colors.length == 1) {
      ...
      barPaint.shader = ui.Gradient.linear(
        Offset(
          barData.invertGradientAxis == true
              ? getLeftOffsetDrawSize()
              : getLeftOffsetDrawSize(),
          barData.invertGradientAxis == true
              ? getTopOffsetDrawSize() + viewSize.height
  ...
  void drawTitles(Canvas canvas, Size viewSize) {
    ...
    if (data.titlesData.showVerticalTitles) {
      ...
      while (verticalSeek <= data.maxY) {
        ...
        double y = getPixelY(verticalSeek, viewSize) + getTopOffsetDrawSize();

        final String text = data.titlesData.getVerticalTitles(verticalSeek);

        final TextSpan span = TextSpan(
            style: data.titlesData.verticalTitlesTextStyle, text: text);
    ...
    if (data.titlesData.showHorizontalTitles) {
      ...
      while (horizontalSeek <= data.maxX) {
        ...
        String text = data.titlesData.getHorizontalTitles(horizontalSeek);

        TextSpan span = TextSpan(
            style: data.titlesData.horizontalTitlesTextStyle, text: text);
